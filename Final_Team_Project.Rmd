...
test_df <- cbind(y_test = y_test, X_test)
```
...
-- We split it into different code blocks so that it can run independently. --
...
```{r}
source("Data/split_data.R")
source("Models/KNN_Model.R")
cm_knn
```

```{r predict_ANN}
source("Data/split_data.R")
source("Models/ANN_Model.R")
```
